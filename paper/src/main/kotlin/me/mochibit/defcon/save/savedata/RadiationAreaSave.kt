...
class RadiationAreaSave private constructor(
    worldName: String,
    useCache: Boolean = true,
) : AbstractSaveData<RadiationSaveSchema>(
        schemaSerializer = serializer(),
        createEmptySchema = { RadiationSaveSchema() },
        useCache = useCache,
    ) {
    ...
    companion object {
        ...
        fun getSave(worldName: String): RadiationAreaSave = saves.computeIfAbsent(worldName) { name -> RadiationAreaSave(name) }
    }

    /**
     * Adds a new radiation area
     */
    suspend fun addRadiationArea(area: RadiationArea): RadiationArea =
     ...
     */
    suspend fun getAll(): Set<RadiationArea> =
        withContext(Dispatchers.IO) {
     ...
     */
    suspend fun get(id: Int): Pair<RadiationArea, Int>? =
        withContext(Dispatchers.IO) {
    ...
    class Builder : AbstractSaveData.Builder<RadiationSaveSchema, RadiationAreaSave>() {
        ...
        override fun build(): RadiationAreaSave = getSave(worldName)
    }
}
