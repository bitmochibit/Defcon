package com.mochibit.nuclearcraft.database
...
abstract class Database {
    ...
    fun connect(databaseFile: DatabaseFile) {
        ...
        val dataFolder: String = NuclearCraft.Companion.instance!!.getDataFolder().getAbsolutePath()
        // Create the db directory if it doesn't exist
        val dbDirectory = "$dataFolder/db"
        val dbDir = File(dbDirectory)
        if (!dbDir.exists()) {
            if (!dbDir.mkdir()) {
                NuclearCraft.Companion.instance!!.getLogger().warning("Failed to create database directory!")
            }
        }
